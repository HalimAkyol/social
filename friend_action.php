...
if(isset($_POST["action"]))
{
	...
	//burası iptal. veriler chat/fetch_user.php dosyasından çekiliyor
	// if ($_POST['action']=='view_list_friends') {
	// 	$output='';
	// 	$array=[];
	// 	$query = "
	// 			SELECT * FROM friend_request 
	// 			INNER JOIN register_user ON friend_request.request_from_id=register_user.register_user_id
	// 			WHERE friend_request.request_to_id = '".$_SESSION["user_id"]."' 
	...
	// 		$output .= '
	// 		<li>
	// 			'.Get_user_avatar($row["request_from_id"], $connect).'&nbsp;
	// 			<button id="user-list" class="btn btn-secondary btn-xs" data-id="'.$row['register_user_id'].'">
	// 			'.view_list_frineds($connect,$_SESSION['user_id'],$row['register_user_id']).'
	// 			</button>
	// 			&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
	// 			<button type="button" name="accept_friend_request_button" class="btn btn-primary btn-xs pull-right accept_friend_request_button"><i class="fa fa-comment" aria-hidden="true"></i> Message</button>
	// 		</li>
	// 		';
	// 	}
	// 	echo $output;
	...
	if($_POST["action"] == 'load_friends')
	{
		...
		$query = "
		SELECT register_user.user_name, friend_request.request_from_id, friend_request.request_to_id 
		FROM register_user 
		INNER JOIN friend_request ON friend_request.request_from_id = register_user.register_user_id OR friend_request.request_to_id = register_user.register_user_id 
		...
		if($statement->rowCount() > 0)
		{
			...
			foreach($statement->fetchAll() as $row)
			{
				...
				<div class="col-md-4" style="margin-bottom:12px;">
					...
					<div align="center"><b><a href="#" style="font-size:12px;">'.get_user_name($temp_user_id,$connect).'</a></b></div>
				</div>
				';
