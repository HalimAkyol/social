...
if(isset($_POST["action"]))
{
	if($_POST['action'] == 'create')
	{
		...
		$query = "
					INSERT INTO posts_table (user_id, post_content, post_code, post_datetime) 
	...
	if($_POST['action'] == 'update')
	{
		...
		$query = " UPDATE posts_table SET post_content = :post_content WHERE posts_id = :post_id ";

		$statement = $connect->prepare($query);

		$statement->execute($data);

		$fetch_media = "SELECT * FROM media_table WHERE post_id = '".$_POST['post_id']."'";
		...
		$output = array(
			...
			'content'		=> $_POST["content"]. '<br /><br /><div id="'.$temp_id.'"></div>',
			'user_image'	=>	Get_user_avatar($_SESSION["user_id"], $connect),
			'user_name'		=>	$_SESSION["user_name"],
