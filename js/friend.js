$(document).ready(function() {
  ...
  list_view_friends();
   ...
   function load_friends_request_list_data()
   {
	...
	$('#chat-sidebar').html('<div align="center" style="line-height:200px;"><i class="fa fa-circle-o-notch fa-spin"></i></div>');
     function list_view_friends(query='') 
    {
            $.ajax({
                url:"chat/fetch_user.php",
                method:"POST",
                data:{action:'view_list_friends',query:query},
                success:function(data) {
    ...
    $(document).on('keyup', '#search_friend', function(){
    	...
    	if(search_value != '')
    	{
    		list_view_friends(search_value);
    	}
    	else
    	{
    		list_view_friends();
    ...
    $('#share_button').click(function(){
        ...
        $.ajax({
          url:"post_action.php",
          method:"POST",
          data:{content:content, action:'create', post_id:post_id},
          dataType:"JSON",
          beforeSend:function()
          {
